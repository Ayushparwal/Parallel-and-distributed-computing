/*
   Program name: 1vecAddUsingOneBlock.cu
  ...
  Contact Number: 7276834418 
    ...
    Description: Vector addition using a single thread block (N <= # threads per block)
                ...
                * To validate results h_c and hr_c are compared
                 ...
                 To compile nvcc -arch=sm_35 1vecAddUsingOneBlock(N=numT).cu
                 To Run     ./a.out
*/
#include <stdio.h>
#include<time.h>
...
__global__ void vecAdd_kernel_UsingManyBlocks(int *d_a, int *d_b, int *d_c, int N)
{
  ...
  if(tid < N) // To avoid out of order memory access by thread
  {
...
int main()
{
  ...
  */
  ...
  cudaMemcpy(d_c, h_c, N*sizeof(int), cudaMemcpyHostToDevice); // Copy data from Host to Device for vector c
