...
__global__ void vecAdd_kernel_UsingSingleBlock(int *d_a, int *d_b, int *d_c, int N)
{
  int tid = threadIdx.x;// threadIdx.x returns thread IDs local to each thread block
 
  for(int i = tid; i < N; i += blockDim.x) // To avoid out of order memory access by thread
  {
     d_c[i] = d_a[i] + d_b[i];
  }
}
...
int main()
{
  ...
  */
  ...
  vecAdd_kernel_UsingSingleBlock<<<1, 1024>>>(d_a, d_b, d_c, N);
